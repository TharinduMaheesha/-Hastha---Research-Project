...
def caption():
	return render_template("SSLCaption_home.html")

@app.route("/translate", methods=['POST', 'GET'])
def greeter():   
    link = str(request.form['text'])
    test , url = 	main.mainFunction(link)
    data = [{"link":url , "video" : test+".mp4"}]
    flash(test+".mp4" , "test")
    return render_template("SSLTranslation.html" , data=data)
...
app.register_blueprint(number,url_prefix="/numbers")
app.register_blueprint(animals,url_prefix="/animals")


@app.route('/ASL' , methods=['POST', 'GET'])
def home():
...
def months():
    ...
    #         sequence = sequence[:30]
        ...
        return (render_template('ASL.html',value=x, mode=mode))
...
def prediction():
    ...
    # return (render_template('ASL.html',value=x))
